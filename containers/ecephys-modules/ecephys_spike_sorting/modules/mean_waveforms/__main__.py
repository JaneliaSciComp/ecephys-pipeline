...
def calculate_mean_waveforms(args):
    ...
    if args['mean_waveform_params']['use_C_Waves']:
        ...
        if args['mean_waveform_params']['calc_half_run']:
            ...
            cwaves_cmd_parts.append(' -snr_radius=' + repr(args['mean_waveform_params']['snr_radius']))
            cwaves_cmd_parts.append(' -snr_radius_um=' + repr(args['mean_waveform_params']['snr_radius_um']))
            cwaves_cmd_parts.append(' -prefix=first_half')
            cwaves_cmd_parts.append(' -startsecs=' + repr(startsecs))
            cwaves_cmd_parts.append(' -endsecs=' + repr(endsecs/2.0))
            ...
            cwaves_cmd_parts.append(' -snr_radius=' + repr(args['mean_waveform_params']['snr_radius']))
            cwaves_cmd_parts.append(' -snr_radius_um=' + repr(args['mean_waveform_params']['snr_radius_um']))
            cwaves_cmd_parts.append(' -prefix=second_half')
            cwaves_cmd_parts.append(' -startsecs=' + repr(endsecs/2.0))
            cwaves_cmd_parts.append(' -endsecs=' + repr(endsecs))
            ...
            create_UnitMatch_input(clu_counts, fh_name, sh_name, args['mean_waveform_params']['pre_samples'],
                                 args['ephys_params']['num_channels']-args['mean_waveform_params']['nAP'], dest)
...
def create_UnitMatch_input(clu_counts, fh_name, sh_name, pre_samples, num_sync, dest ):
    # parse mean_waveform files from the first and second half of run for 
    # input to UnitMatch
    # clu_ounts = array of counts for each spike label, 0 to maximum spike label
    # fh_name = name for mean_waveforms.npy file for first half
    # sh_name = name for mean_waveforms.npy file for the 2nd half
    # pre_samples = same as sent to C_waves. Used here to define where to estimate baseline
    # dest = phy directory of output, inclluding the means_waveforms files
    # and the RawWaveforms directory
    #
    # output waveforms background subtracted and the sync channel is removed
    #
    # Note: Bombcell and UM native calculation of mean waveforms includes 
    # smoothing each waveform over time -- unnecesssary with filtered data
    # Baseline subtraction is also unnecesssary for filtered data
