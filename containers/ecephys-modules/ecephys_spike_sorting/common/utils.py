...
def load_kilosort_data(folder,
    ...
    for temp_idx in range(templates.shape[0]):
        ...
        unwhitened_temps[temp_idx, :, :] = np.dot(np.ascontiguousarray(
            templates[temp_idx, :, :]), np.ascontiguousarray(unwhitening_mat))

    try:
        cluster_ids, cluster_quality = read_cluster_group_tsv(
            os.path.join(folder, 'cluster_group.tsv'))
    except OSError:
...
def get_spike_depths(spike_clusters, unit_template_ids, first_pc_sq, pc_feature_ind, channel_pos):
    ...
    spike_depths = np.sum(spike_feat_ycoord * first_pc_sq,
                          1) / np.sum(first_pc_sq, 1)
...
def get_spike_amplitudes(spike_templates, templates, amplitudes):
    ...
    template_amplitudes = np.max(
        np.max(templates, 1) - np.min(templates, 1), 1)
...
def catGT_ex_params_from_str(ex_str):
    ...
    if ex_type == 'SY' or ex_type == 'iSY':
        ...
        if ex_parts[1] == '-1':
            ex_parts[1] = '*'
        ex_name_str = ex_type + '_' + \
            ex_parts[1] + '_' + ex_parts[2] + '_' + ex_parts[3]
