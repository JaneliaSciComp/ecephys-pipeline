...
def run_kilosort(args):
    ...
    if args['kilosort_helper_params']['spikeGLX_data']:
        ...
        _, offset, scaling, surface_channel, air_channel = read_probe_json(
            args['common_files']['probe_json'])

        mask[args['ephys_params']['reference_channels']] = False

        top_channel = np.min([args['ephys_params']['num_channels'], int(
            surface_channel) + args['kilosort_helper_params']['surface_channel_buffer']])
    ...
    # copy the master fle to the same directory that contains the channel map and config file
    master_fullpath = os.path.join(os.path.join(
        args['kilosort_helper_params']['master_file_path'], args['kilosort_helper_params']['master_file_name']))
    ...
    master_call = os.path.splitext(
        args['kilosort_helper_params']['master_file_name'])[0]
...
def get_noise_channels(raw_data_file, num_channels, sample_rate, bit_volts, noise_threshold=20):
    ...
    noise_interval = 10  # in

    print('Mapping raw data file:', raw_data_file)
    ...
    print('number of noise channels: ' +
