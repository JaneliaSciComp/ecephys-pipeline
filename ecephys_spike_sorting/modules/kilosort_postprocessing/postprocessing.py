...
def remove_double_counted_spikes(spike_times, spike_clusters, spike_templates,
    ...
    spikes_to_remove = np.zeros(len(spike_times), dtype=bool)
    ...
    for idx1, unit_id1 in enumerate(sorted_unit_list):
        ...
        spikes_to_remove[for_unit1[to_remove]] = True

        printProgressBar(idx1+1, len(sorted_unit_list))
    ...
    for idx1, unit_id1 in enumerate(sorted_unit_list):
        ...
        for idx2, unit_id2 in enumerate(unit_list[order]):
            deltaX = np.squeeze(
                channel_pos[peak_chan_idx[unit_id2], 0] - channel_pos[peak_chan_idx[unit_id1], 0])
            deltaZ = np.squeeze(
                channel_pos[peak_chan_idx[unit_id2], 1] - channel_pos[peak_chan_idx[unit_id1], 1])
            ...
            if idx2 > idx1 and dist < params['between_unit_dist_um']:
                ...
                to_remove1, to_remove2 = find_between_unit_overlap(
                    spike_times[for_unit1], spike_times[for_unit2], amp1, amp2, between_unit_overlap_samples, params['deletion_mode'])

                overlap_matrix[idx1, idx2] = overlap_matrix[idx1,
                                                            idx2] + len(to_remove1)
                overlap_matrix[idx2, idx1] = overlap_matrix[idx2,
                                                            idx1] + len(to_remove2)

                spikes_to_remove[for_unit1[to_remove1]] = True
                spikes_to_remove[for_unit2[to_remove2]] = True
    ...
    for idx1, unit_id1 in enumerate(unit_list[order]):
        ...
        overlap_summary[idx1, 3] = np.sum(
            overlap_matrix[idx1, :]) - overlap_matrix[idx1, idx1]
        overlap_summary[idx1, 4] = sorted_unit_list[np.argmax(
            overlap_matrix[idx1, :])]
...
def find_between_unit_overlap(spike_train1, spike_train2, amp1, amp2, overlap_window=5, deletionMode='lowAmpCluster'):
    ...
    cluster_ids = np.concatenate(
        (np.zeros((len(spike_train1),)), np.ones((len(spike_train2),))))
    ...
    if deletionMode == 'deleteFirst':
        ...
        spikes_to_remove1 = sorted_inds[spikes_to_remove *
                                        (sorted_cluster_ids == 0)]
        spikes_to_remove2 = sorted_inds[spikes_to_remove *
